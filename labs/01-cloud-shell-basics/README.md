# 01 - Erstes Arbeiten mit Cloud Shell

## Schritt 1: Shell öffnen

Starten Sie Cloud Shell über den Button oben in der Menüleiste

![open-shell.png](images/open-shell.png)

Es wird nun einige Zeit dauern, bis die Shell einsatzbereit ist. Sobald Sie
eine Eingabeaufforderung erhalten wie im nächsten Bild gezeigt, können Sie
fortfahren.
...
Prüfen Sie nach jedem Befehl das Ergebnis und schauen Sie auf die verlinkten
Artikel in der Cloud SDK-Dokumentation.

```shell
gcloud --version
# https://cloud.google.com/sdk/gcloud/reference/config/list
gcloud config list
# https://cloud.google.com/sdk/gcloud/reference/config/get
...
Dateien hoch- und herunterladen lassen, ohne vorher Speicherplatz angefragt
haben zu müssen.
...
Bucket-Namen müssen in Cloud Storage global eindeutig sein, daher haben wir
einen zufälligen Namensteil eingebaut.
...
Geben Sie in der Suchleiste "Cloud Storage" ein und wählen Sie den Menüpunkt
aus, der erscheint.
Es erscheint eine Liste aller Buckets des aktuellen Projekts.

![gcs-bucket-list.png](images/gcs-bucket-list.png)

Wählen Sie den gerade angelegten Bucket aus. Die Datei sollte sich nun in der
Liste wiederfinden.

![gcs-file-list.png](images/gcs-file-list.png)
